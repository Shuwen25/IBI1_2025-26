...
seq_length=len(seq)
for start_point in range(seq_length-2):
    for current_point in range(start_point,seq_length-2,3):
        triplet=seq[current_point:current_point+3]
        if triplet==start_codon:
            #check if the triplet start from the current index is the start codon
            for current_idx in range(current_point,seq_length-2,3):
                ...
                if current_triplet in stop_codons:
                    orf_seq=seq[current_point:current_idx+3]
